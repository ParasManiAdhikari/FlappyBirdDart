...
class View {
  ...
  void update() {
    ...
    var pipeDownLeft = (pipeDown?.getBoundingClientRect().left.toInt() ?? 0) -
        fieldLeft; //subtract fieldLeft because it is different for different screen resolutions
    var ballBottom = ballTop + 35;
    var collision = (ballTop < newField.pipes.pipeUpBottom &&
            pipeUpLeft < 150 &&
            pipeUpLeft > 50) ||
        (ballBottom > newField.pipes.pipeDownTop &&
            pipeDownLeft < 150 &&
            pipeDownLeft > 50);
...
void main() {
  ...
  sbutton?.onClick.listen((_) async {
    //if start clicked run the main loop
    gamestate = #on;
